...
class Dimension extends DataObject
{
	 ...
	 */
	function createNew($blockId, $title, $description, $reference_value = NULL, $reference_value_type = NULL)
	{
		$db = &$GLOBALS['dao']->getConnection();
		$id = $db->nextId(DB_PREFIX.'dimensions');
		$res = $db->query("INSERT INTO ".DB_PREFIX."dimensions VALUES (?, ?, ?, ?, ?, ?, ?)",
			array($id, $title, $description, $blockId, $reference_value, $reference_value_type, 0));
